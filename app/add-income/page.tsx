    ...
    const loadBudgets = async () => {
      try {
        ...
        if (Array.isArray(data.budgets) && data.budgets.length > 0) {
          setAllocations(
            data.budgets.map((budget: { id: string; name: string; percent: number }) => ({
              id: budget.id,
              name: budget.name,
  ...
  return (
    <DashboardShell>
      <div className="flex flex-col gap-10">
        <ScreenHeader
          title="Add Income"
          subtitle="Capture income quickly."
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <Button size="lg" disabled={!canSubmit || isSubmitting} onClick={handleSave}>
