...
export async function GET() {
  ...
  return new Response(JSON.stringify({ user, budgets }), {
    status: 200,
...
export async function PUT(request: Request) {
  ...
  const conversionRate = parseNumber(body.conversionRate ?? 0);
  const imageKey =
  ...
  if (!conversionRate || conversionRate <= 0) {
    return new Response("Conversion rate must be greater than zero", {
      status: 400,
    });
  }
